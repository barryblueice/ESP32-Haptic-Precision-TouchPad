...
void device_config_defaults(device_config_t *c)
{
    *c = (device_config_t){{63, 2, 80, 100, 130, 0, 1, 0}};
    ...
    for (unsigned i = CFG_EDGES; i < DEVICE_CONFIG_SIZE; i += 5) { c->bytes[i + 3] = 5; c->bytes[i + 4] = 2; }
}
...
{
    const uint8_t header[8] = {'R','S','C','F',2,0,52,0};
...
bool device_config_supported(const device_config_t *a, const device_config_t *b, uint32_t caps)
{
    ...
    if (!(caps & RSTP_CAP_POINTS)) {
        ...
        if (memcmp(b->bytes + CFG_POINTS, defaults.bytes + CFG_POINTS, 20) || (b->bytes[7] & 0xf0)) return false;
    }
